...
class AioWithImapServer(WithImapServer):
    async def login_user(self, login, password, select=False, lib=aioimaplib.IMAP4):
...
class TestAioimaplib(AioWithImapServer, asynctest.TestCase):
    ...
    async def test_fetch_by_uid_without_body(self):
        ...
        self.assertEqual('1 FETCH (UID 1 FLAGS ())', response.lines[0])
    ...
    async def test_idle(self):
        ...
        self.assertEqual(['1 EXISTS', '1 RECENT'], (await imap_client.wait_server_push()))

        imap_client.idle_done()
        self.assertEqual(('OK', ['IDLE terminated']), (await asyncio.wait_for(idle, 1)))
    ...
    async def test_store_and_search_by_keyword(self):
        ...
        self.imapserver.receive(Mail.create(['user']))
        imap_client = await self.login_user('user', 'pass', select=True)
        self.assertEqual('', (await imap_client.uid_search('KEYWORD FOO', charset=None)).lines[0])

        self.assertEqual('OK', (await imap_client.uid('store', '1', '+FLAGS (FOO)')).result)

        self.assertEqual('1', (await imap_client.uid_search('KEYWORD FOO', charset=None)).lines[0])
        self.assertEqual('2', (await imap_client.uid_search('UNKEYWORD FOO', charset=None)).lines[0])
    ...
    async def test_expunge_messages(self):
        ...
        self.assertEqual(('OK', ['1 EXPUNGE', '2 EXPUNGE', 'EXPUNGE completed.']), (await imap_client.expunge()))
    ...
    async def test_concurrency_3_executing_async_commands_in_parallel(self):
        ...
        self.assertEqual('1', (await imap_client.search('KEYWORD FOO', charset=None)).lines[0])
    ...
    async def test_noop(self):
        imap_client = await self.login_user('user', 'pass')
        self.assertEqual(('OK', ['NOOP completed.']), (await imap_client.noop()))

    async def test_noop_with_untagged_data(self):
        imap_client = await self.login_user('user', 'pass')
        self.imapserver.receive(Mail.create(['user']))

        self.assertEqual(('OK', ['1 EXISTS', '1 RECENT', 'NOOP completed.']), (await imap_client.noop()))

    async def test_check(self):
        imap_client = await self.login_user('user', 'pass', select=True)
        self.assertEqual(('OK', ['CHECK completed.']), (await imap_client.check()))

    async def test_close(self):
        imap_client = await self.login_user('user', 'pass', select=True)
        self.assertEqual(imapserver.SELECTED, self.imapserver.get_connection('user').state)

        self.assertEqual(('OK', ['CLOSE completed.']), (await imap_client.close()))
    ...
    async def test_status(self):
        ...
        self.assertEqual('INBOX (MESSAGES 0 UIDNEXT 1)',
                          (await imap_client.status('INBOX', '(MESSAGES UIDNEXT)')).lines[0])

    async def test_subscribe_unsubscribe_lsub(self):
        imap_client = await self.login_user('user', 'pass')

        self.assertEqual(('OK', ['SUBSCRIBE completed.']), (await imap_client.subscribe('#fr.soc.feminisme')))
        self.assertEqual(('OK', ['() "." #fr.soc.feminisme', 'LSUB completed.']),
                          (await imap_client.lsub('#fr.', 'soc.*')))
        self.assertEqual(('OK', ['UNSUBSCRIBE completed.']), (await imap_client.unsubscribe('#fr.soc.feminisme')))
        self.assertEqual(('OK', ['LSUB completed.']), (await imap_client.lsub('#fr', '.*')))

    async def test_create_delete_mailbox(self):
        imap_client = await self.login_user('user', 'pass')
        self.assertEqual('NO', (await imap_client.status('MBOX', '(MESSAGES)')).result)

        self.assertEqual(('OK', ['CREATE completed.']), (await imap_client.create('MBOX')))
        self.assertEqual('OK', (await imap_client.status('MBOX', '(MESSAGES)')).result)

        self.assertEqual(('OK', ['DELETE completed.']), (await imap_client.delete('MBOX')))
        self.assertEqual('NO', (await imap_client.status('MBOX', '(MESSAGES)')).result)
    ...
    async def test_rename_mailbox(self):
        ...
        self.assertEqual(('OK', ['RENAME completed.']), (await imap_client.rename('INBOX', 'MBOX')))

        self.assertEqual('OK', (await imap_client.status('MBOX', '(MESSAGES)')).result)

    async def test_list(self):
        imap_client = await self.login_user('user', 'pass')
        self.assertEqual(('OK', ['() "/" Drafts', '() "/" INBOX', '() "/" Sent', '() "/" Trash',
                                  'LIST completed.']), (await imap_client.list('""', '.*')))

        await imap_client.create('MYBOX')
        self.assertEqual(('OK', ['() "/" Drafts', '() "/" INBOX', '() "/" MYBOX', '() "/" Sent', '() "/" Trash',
                                  'LIST completed.']),
    ...
    async def test_rfc5032_within(self):
        ...
        self.assertEqual('1', (await imap_client.search('OLDER', '84700')).lines[0])
        self.assertEqual('2 3', (await imap_client.search('YOUNGER', '84700')).lines[0])
    ...
    async def test_rfc4315_uidplus_expunge(self):
        ...
        self.assertEqual(('OK', ['1 EXPUNGE', 'UID EXPUNGE completed.']), (await imap_client.uid('expunge', '1:1')))
...
class TestAioimaplibClocked(AioWithImapServer, asynctest.ClockedTestCase):
    ...
    async def test_idle_start__exits_queueget_without_timeout_error(self):
        ...
        await self.advance(TWENTY_NINE_MINUTES + 1)
